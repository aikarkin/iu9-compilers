...
// Tree
    ...
    template <class T, class tree_node_allocator>
    tree<T,tree_node_allocator>& tree<T, tree_node_allocator>::operator=(tree<T, tree_node_allocator>&& rst)
    {
        if (this != &rst)
        {
            head->next_sibling                     = rst.head->next_sibling;
            feet->prev_sibling                     = rst.head->prev_sibling;
            rst.head->next_sibling->prev_sibling   = head;
            rst.feet->prev_sibling->next_sibling   = feet;
            rst.head->next_sibling                 = rst.feet;
            rst.feet->prev_sibling                 = rst.head;
        }
        return *this;
    }
    ...
    template <class T, class tree_node_allocator>
    typename tree<T, tree_node_allocator>::sibling_iterator tree<T, tree_node_allocator>::replace(sibling_iterator orig_begin,
                                                                                                  sibling_iterator orig_end,
                                                                                                  sibling_iterator new_begin,
                                                                                                  sibling_iterator new_end)
    {
...
// specialization for sibling_iterators
    template <class T, class tree_node_allocator>
    ...
    {
        if (other.head->next_sibling == other.feet)  // other tree is empty
        {
            return loc;
        }
...
// }
    ...
    template <class T, class tree_node_allocator>
    typename tree<T, tree_node_allocator>::iterator tree<T, tree_node_allocator>::lowest_common_ancestor(const iterator_base& one,
